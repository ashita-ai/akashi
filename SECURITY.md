...
- **Credential management**: Database credentials and API keys are provided via environment variables. No credentials are stored in code. `.env` files are gitignored.
...
- Compromise of the PostgreSQL instance itself (the application assumes it is access-restricted).
