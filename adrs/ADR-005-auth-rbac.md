...
| Role | Rank | Purpose |
|------|------|---------|
| `platform_admin` | 5 | Cross-org operations, system configuration, administrative tasks |
...
- **Platform operators** (`platform_admin`) need cross-org visibility for support and system health. Collapsing this into `admin` would leak cross-org access to organization-level administrators.
...
- Every API request (except `/health` and `/auth/token`) requires a valid Bearer token in the Authorization header.
