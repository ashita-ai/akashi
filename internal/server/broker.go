...
type Broker struct {
	...
	droppedEvents metric.Int64Counter // events dropped due to unparseable org_id or slow subscribers
}

// NewBroker creates a new SSE broker. Call Start to begin listening.
func NewBroker(db *storage.DB, logger *slog.Logger) *Broker {
	meter := telemetry.Meter("akashi/broker")
	dropped, _ := meter.Int64Counter("akashi.broker.dropped_events",
		metric.WithDescription("Events dropped due to unparseable org_id or slow subscribers"),
	)
	return &Broker{
		db:            db,
		logger:        logger,
...
// The close is performed under the lock to prevent a race with broadcastToOrg
// sending on a channel that has been deleted from the map but not yet closed.
