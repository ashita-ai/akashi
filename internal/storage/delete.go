...
func (db *DB) DeleteAgentData(ctx context.Context, orgID uuid.UUID, agentID string, audit *MutationAuditEntry) (DeleteAgentResult, error) {
	...
	txErr := db.WithTx(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("storage: archive conflict resolutions for delete: %w", err)
		}
		...
		if _, err = tx.Exec(ctx, `SET LOCAL akashi.allow_conflict_resolution_delete = 'true'`); err != nil {
			return fmt.Errorf("storage: set conflict resolution delete flag: %w", err)
		}
		...
		tag, err = tx.Exec(ctx,
		 ...
		 WHERE org_id = $1
		   AND conflict_id IN (
			...
			orgID, agentID)
		if err != nil {
			return fmt.Errorf("storage: delete conflict resolutions: %w", err)
		}
		...
		if _, err = tx.Exec(ctx, `SET LOCAL akashi.allow_conflict_resolution_delete = 'false'`); err != nil {
			return fmt.Errorf("storage: reset conflict resolution delete flag: %w", err)
		}
