...
func (l *LiteDB) GetConfidenceDistribution(ctx context.Context, orgID uuid.UUID, from, to *time.Time) (storage.ConfidenceDistribution, error) {
	...
	var highConfCompletenessSum float64
	...
	q := `SELECT
		     ...
		     COALESCE(SUM(CASE WHEN confidence >= 0.85 THEN completeness_score ELSE 0 END), 0),
		     COALESCE(SUM(CASE WHEN confidence >= 0.85 THEN 1 ELSE 0 END), 0)
		 FROM decisions
	...
	err := l.db.QueryRowContext(ctx, q, args...).Scan(
		...
		&highConfCompletenessSum, &highConfCount,
	)
	if err != nil {
	...
	if highConfCount > 0 {
		d.HighConfAvgCompleteness = highConfCompletenessSum / float64(highConfCount)
	}
