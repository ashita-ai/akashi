...
// channels on success. Must be called with db.notifyMu held. The mutex is
// released during backoff sleeps to avoid blocking SSE subscribers.
func (db *DB) reconnectNotify(ctx context.Context) error {
	...
	for attempt := range maxRetries {
		if attempt > 0 {
			...
			// Release the mutex during backoff sleep so WaitForNotification
			// callers aren't blocked for the full reconnect duration.
			db.notifyMu.Unlock()
