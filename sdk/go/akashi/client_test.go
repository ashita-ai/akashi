...
func TestGetUsage(t *testing.T) {
	...
	srv := mockServer(t, map[string]http.HandlerFunc{
		"GET /v1/usage": func(w http.ResponseWriter, r *http.Request) {
			writeJSON(w, http.StatusOK, map[string]any{
				"data": UsageResponse{
					OrgID:         orgID,
					Plan:          "pro",
	...
	if usage.OrgID != orgID {
		t.Errorf("expected org_id %s, got %s", orgID, usage.OrgID)
	}
	if usage.Plan != "pro" {
		t.Errorf("expected plan 'pro', got %q", usage.Plan)
	}
	if usage.DecisionCount != 42 {
		t.Errorf("expected decision_count 42, got %d", usage.DecisionCount)
	}
	if usage.DecisionLimit != 10000 {
		t.Errorf("expected decision_limit 10000, got %d", usage.DecisionLimit)
	}
}
